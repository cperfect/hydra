...
func (d *clientSqlData) ToClient() *client.Client {
	log.Printf("Client sql data %#v", d)
...
func (m *ClientManager) GetConcreteClient(ID string) (*client.Client, error) {
	var d clientSqlData
	log.Printf("get concrete %s", m.GetTable())
	//if err := m.DB.Get(&d, m.DB.Rebind(fmt.Sprintf("SELECT  ID, CLIENT_NAME, CLIENT_SECRET, REDIRECT_URIS, GRANT_TYPES, RESPONSE_TYPES, \"SCOPE\", OWNER, POLICY_URI, TOS_URI, CLIENT_URI, LOGO_URI, CONTACTS, IS_PUBLIC FROM %s WHERE ID=?", m.GetTable())), ID); err == sql.ErrNoRows {
